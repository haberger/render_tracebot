...
def render(config):#, q=None):
    ...
    for i in range(config["num_scenes"]):
        ...
        for bop_dataset in bop_datasets.values():
            dist_per_datatset = min(config["objects"]["num_distractions"], len(bop_dataset))
        ...
        while cam_poses < config["img_per_scene"]:
            ...
            poi = bproc.object.compute_poi(np.random.choice(sampled_target_objs, size=max(1, len(sampled_target_objs)-2), replace=False))
        ...
        if set_path != old_set_path:
            # q.put(old_set_path)
            old_set_path = set_path
