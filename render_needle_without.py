...
def render(config):
    ...
    for i in range(config["num_scenes"]):
        ...
        while cam_poses < config["img_per_scene"]:
            ...
            poi = bproc.object.compute_poi(np.concatenate((np.random.choice(sampled_distractor_bop_objs, size=max(1, len(sampled_distractor_bop_objs)-1), replace=False), np.array(sampled_target_objs))))
